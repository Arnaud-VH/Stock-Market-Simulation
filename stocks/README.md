...
- ####networking Module
The `networking` Module is a library like module that has the implementation of server-client communication. This module is independent of TraderClient and ServerExchange implementation that we use later on. 
This module is used in the `network-market` module to facilitate the communication over the network of the Exchange and the Trader. In the `networking` module we also define the `networkMessage` class that is a message which is sent over the network. This class is important as it is used in many other modules, like the `network-market` module. There it is used to wrap a command in a networking message so that it can be sent across the network. 
